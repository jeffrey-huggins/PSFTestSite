...
function InitializeDates() {
    $(".isDate").each(function (index, element) {
        if ($(element).val().indexOf("1/1/0001") > -1) {
            $(element).val("");
        }
...
function PreparePage() {
    ...
    formContent.on('click', '#submitPurchaseOrder', function (e) {
        ...
        var url = $("#SavePurchaseOrderItem").attr("action");
        //$.ajax({
        //    url: url,
        //    type: "POST",
        //    data: $("#SavePurchaseOrderItem").serialize(),
    ...
    formContent.on("change", "#VendorId", function (e) {
        ...
        if (vendorId != "") {
            $.ajax({
                url: path + "./PurchaseOrder/GetVendor",
                dataType: "json",
                cache: false,
                type: "GET",
